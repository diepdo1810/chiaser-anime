...
// Updated types to match oManga API structure
type MangaData = {
  id: string;
  title: string;
  coverImage: {
...
export default function WriteCommentFormContainer({
  ...
  async function handleCreateComment(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!isCommentTextValid(form.comment.value)) return;
              ...
              ...[
                {
                  ...
                  media: {
                    id: mediaInfo.id,
                    coverImage: {
                      // check undefined here, if it is, use a fallback image
                      large: "",
                    },
                    title: "",
                  },
      ...
      >
        <label>
          {(user || anilistUser) &&
            `${
              isAReply
                ? `Reply as ${user?.displayName || anilistUser?.name}`
                : `Comment as ${user?.displayName || anilistUser?.name}`
            }`}
