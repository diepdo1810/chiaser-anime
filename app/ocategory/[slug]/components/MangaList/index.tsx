...
import { MangaCategoryResponse } from "@/app/ts/interfaces/apiOMangaDataInterface";
...
const MangaList: React.FC<MangaListProps> = ({ slug, page = 1 }) => {
  ...
  useEffect(() => {
    const fetchComics = async () => {
      setLoading(true);
      try {
        const response = await omanga.getComicsByCategory({
          slug,
          page: currPageNumber,
        });
  ...
  return (
    <div id={styles.content_container}>
          ...
          <AnimatePresence initial={true} mode="wait">
            ...
            >
              {comics
                .filter(
                  (comic, index, self) =>
                    index === self.findIndex((c) => c._id === comic._id)
                )
                .map((comic, key) => (
                  <motion.li key={comic._id} variants={framerMotionShowUp}>
                    <MediaCard.Container onDarkMode>
                      ...
                      <MediaCard.LinkTitleOmanga title={comic.name} />
                    </MediaCard.Container>
                  </motion.li>
                ))}
            </motion.ul>
      ...
      {comics && currPageNumber < totalPages && (
        <button
      ...
      {comics && comics.length > 0 && (
        <span>
          Showing{" "}
          {Math.min(currPageNumber * 24, totalLength) === totalLength
            ? "all "
            : `${Math.min(currPageNumber * 24, totalLength)} out of `}
