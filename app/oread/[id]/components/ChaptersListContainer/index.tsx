...
function ChaptersListContainer({ mangaSlug, currChapterId, chaptersList }: ComponentTypes) {
    useEffect(() => {
            ...
            <motion.ol id={styles.list_container}>
                ...
                {chaptersList.server_data.map((chapter: MangaChapters, key: number) => (
                    <motion.li
                        ...
                        key={key}
                        data-active={chapter.chapter_api_data.split('/').pop() === currChapterId}
                    >

                        <Link href={`/oread/${chapter.chapter_api_data.split('/').pop()}?slug=${mangaSlug}`}>
